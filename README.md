# annoy-sagemaker-container
based on aws sagemaker bring your own container model
...
4. __[annoy][annoy]__ lists python api for k nearest neighbours search.
...
* __annoy__: The directory that contains the application to run in the container. See the next session for details about each of the files.
...
* __annoy_helper.py__: Modules for annoy prediction
...
* __feature_dim__: dimension length of feature_vector. __Default__: 80
* __nns__: n nearest neighbours (knn). __Default__: 100
* __no_of_trees__: builds a forest of __no_of_trees__ trees. More trees gives higher precision when querying. __Default__: 300
* __metric__: Metric can be "angular", "euclidean", "manhattan", "hamming", or "dot". __Default__: "angular"
* __on_disk__: prepares annoy to build the index in the specified file instead of RAM __Default__: True
* __output_path__: e.g output path(s3 prefix in bucket 'data-science-meesho') where knn of items will be stored in CSV format. __e.g__. "output_path": "rosah/annoy-knn"
* __search_k__: During the query it will inspect up to __search_k__ nodes which defaults to __n_trees * n__ if not provided. __search_k__ gives you a run-time tradeoff between better accuracy and speed.
* __knn_all__: if True knn query for all items. otherwise pass item vectors in test_data parameter __Default__: True
* __search_by_vector__: True if knn by passing vectors __Default__: False
